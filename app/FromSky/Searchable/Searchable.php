<?php namespace App\FromSky\Searchable;
 ...
 * @package App\FromSky\Searchable
 */
trait SearchableTrait
{
    ...
    /**
     * Trả ra thêm theo modeltype
     * @param $objBuilder
     */
    public function withContext($objBuilder){
        if(request()->has("context_field") && request()->has("context_model")){
            $field = request()->input("context_field");
            $model = "App\\Model\\".request()->input("context_model");
            return $objBuilder->where($field, $model);
        }else{
            return "";
        }
