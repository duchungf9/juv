...
use App\Model\Page;
use App\FromSky\Website\Repos\Page\DbPageRepository;
...
class SeoLandingHelper
{
    ...
    {
        $route = config('fromSky.website.seolanding.' . $setting . '.route');
        $route = str_replace('/', '', $route);
        if (!$route) {
            return collect([]);
        }

        $articleRepo = new DbPageRepository(new Page);
        $page        = $articleRepo->getBySlug($setting);
        ...
        foreach ($matches[1] as $_match) {
            ...
            $_values = collect(config('fromSky.website.seolanding.' . $setting . '.' . $_match));
