...
class StringHelper
{
    ...
    public static function buildMenuFromContent($string){
        $regex_string = "/<h([0-9])>((.*)+)<\/h[0-9]>+/";
        $matched = preg_match_all($regex_string, $string,$out, PREG_PATTERN_ORDER);
        $headings = $out[1];
        $minHeading = 1; // quy định h2 là heading chính
        $currentHeading = $minHeading;
        $menu = [];
        foreach($headings as $key=>$heading){
            $currentHeading = $heading;
            if($heading == $minHeading && $currentHeading == $minHeading){ // nếu current = min thì tạo thằng mới
                $menu[] = ['title'=> $out[2][$key] , 'childs'=>[]];
            }
            if($currentHeading > $minHeading){
                // trường hợp nhét thằng con vào thằng cha gần nhất
                $menu[count($menu)-1]['childs'][] = ['title'=>$out[2][$key]];
            }
        }
