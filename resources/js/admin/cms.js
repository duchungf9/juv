window.urlAjaxHandlerCms = _SERVER_PATH + '/api/'; // percorso  del contenuto del  dialog
...
window.Cms = function () {
    ...
    function listHandler() {
        ...
        $('[data-role="delete-item"]').on('click', function (e) {
            ...
            bootbox.confirm(`<h4>Bạn có chắc chắn muốn xóa "${$(curItem).data('title')} ?"</h4>`, function (confirmed) {
                if (confirmed) {
    ...
    return {
        ...
        initTabManager:function(elem){

            let url = location.href.replace(/\/$/, '');
            if (location.hash) {
                const hash = url.split('#');
                const currentTab = document.querySelector('#'+elem+' a[href="#' + hash[1] + '"]');
