@forelse($page->media()->where('collection_name', 'images')->orderBy('sort')->get() as $media)
	<li id="box_media_{{ $media->id}}" class="img-thumbnail">
		...
		<img src="{{ ImgHelper::init('')->get_cached($media->file_name, config('fromSky.image.admin')) }}" alt="{{ $media->title}}" border="0">
		<div class="actions">
